...
export default function Background({ weather }) {
  const [bgClass, setBgClass] = useState('bg-white'); // default white background
  ...
  useEffect(() => {
    ...
    // Adjust times using timezone offset (in seconds)
    const timezoneOffset = weather.timezone || 0; // seconds
    const localTimestamp = (weather.dt + timezoneOffset) * 1000;
    const localDate = new Date(localTimestamp);
    const localHour = localDate.getUTCHours(); // still UTC, but offset handled
    ...
    // Time-based gradient
    let baseColor = 'bg-gradient-to-b from-gray-800 to-black'; // fallback night
    ...
    // Weather-specific overlay (used for additional effects if needed)
    let animationClass = '';
